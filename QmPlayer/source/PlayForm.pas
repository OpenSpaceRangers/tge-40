...
uses
  ...
  ParameterDeltaClass,
  TextFieldClass,
  ExtCtrls, EC_BlockPar, Buttons, ComCtrls;
...
type
  TForm6 = class(TForm)
    ...
    function FindStartLocation:boolean;
    procedure StartGame;
    procedure MakeDo;
    procedure MakeUndo;
...
//    procedure MakeUndo;
  ...
  public
    { Public declarations }
     M:PM;
     PlayGame, NonFixStringedGame:TGame;
     ...
     ButtonCalback:TButtonCalback;
      ...
      AnswerLabels: array[1..8] of TLabel;

      ParamsShowLabels:array [1..3] of TLabel;
...
procedure Tform6.FailMessage;
begin
      ClearAnswers;
      SDTRichEdit.Text:='Квест провален. Можете попробовать сыграть снова.';
      ParLabel1.Caption:='';

end;

procedure Tform6.SuccessMessage;
var dayspassedstr:string;
begin
      ClearAnswers;

      SDTRichEdit.Text:='Вы успешно прошли квест. Если очень понравилось, можете сыграть снова.';
      ParLabel1.Caption:='';
     ...
     SDTRichEdit.Text:=SDTRichEdit.Text+dayspassedstr;
end;
...
//     PlayGame.Pars[i].CriticalMessage.Text:=FixStringParameters(PlayGame.Pars[i].CriticalMessage.Text,false);
    end;
 {
     ...
     for i:=1 to PlayGame.LocationsValue do begin
      ...
      for c:=1 to maxparameters do begin
       PlayGame.Locations[i].DPars[c].CriticalMessage.Text
        :=FixStringParameters(PlayGame.Locations[i].DPars[c].CriticalMessage.Text,true);
      end;
     ...
     //Инициализируем матрицу переходов куда можно пойти
       for i:=1 to PlayGame.LocationsValue do
        ...
        for i:=1 to PlayGame.PathesValue do begin
         if PlayGame.Pathes[i].PassTimesValue=0 then PathesWeCanGo[PlayGame.GetLocationIndex(PlayGame.Pathes[i].FromLocation)][i]:=2000000000
         else PathesWeCanGo[PlayGame.GetLocationIndex(PlayGame.Pathes[i].FromLocation)][i]:=PlayGame.Pathes[i].PassTimesValue;
        end;
    //
...
function TForm6.FindStartLocation:boolean;
var i,c,cln:integer;
    answer:boolean;
begin
      answer:=true;
// Ищем стартовую локацию (
         ...
         for i:=1 to PlayGame.LocationsValue do
          if PlayGame.Locations[i].StartLocationFlag then begin
           cln:=PlayGame.Locations[i].LocationNumber;
           inc(c);
          end;
...
// Если стартовых локаций больше одной, то обработать ошибку
    if cln=-1 then begin
       ...
       if PlayGame.IsNoMeanCanPlay then begin

       end
       else ShowMessage('Существуют тупиковые незначимые локации!');
    end;

    FindStartLocation:=answer;
end;
...
then begin answer:=true;
                 CustomCriticalMessage:=
                  trim(PlayGame.Locations[location].dpars[i].CriticalMessage.text);
                 CurrentCriticalParameter:=i;
                 break;
     end;
...
begin
     for i:=1 to maxparameters do begin
         ...
         if delta[i].ParameterViewAction=HideParameter then
            begin
            ParVisState[i]:=false;
            end;
...
if checkedanswer<>oldcheckanswer then begin
	...
	if (CheckedAnswer>0) and (CheckedAnswer<9) then begin
...
procedure Tform6.AddAnswer(PathIndex:integer; txt:widestring; enabled:boolean);
   var i,theight, height,stdwidth:integer;
begin
    if accessible_pathes_count>=8 then exit;    
    stdwidth:=707;
    for i:=1 to 8 do AnswerLabels[i].Visible:=false;
...
begin
    ...
    //Проверка на достижение финальной локации
        ...
        ClearAnswers;
        AddAnswer(0,'Конец квеста',true);
        ButtonCalback:=SuccessMessage;
    end else
    if (PlayGame.Locations[locationindex].FailLocationFlag) then begin
		//Проверка на достижение провальной локации
		GameState:=GameStateOK;
    	ClearAnswers;
    	AddAnswer(0,'Игра кончилась',true);
    	ButtonCalback:=FailMessage;
    end else begin
...
begin
    ...
    ClearAnswers;
    if proceed_clicked then exit;
    AddAnswer(0,'Квест успешно завершен',true);
    ButtonCalback:=SuccessMessage;
...
begin
    ...
    if ShowPathes(true)<>0 then MakeUndo;
end;
...
var filename,s1:string;
    ...
    i:integer;
begin
	TempGame:=TGame.Create(GameContent.PM(m));
    Path:=TTextField.Create;

	if FileExists(ConfigFilePath) then begin
    	AssignFile(f,ConfigFilePath);
        Reset(f,1);
        path.Load(@f);
        closefile(f);
        OpenDialog1.InitialDir:=trim(path.Text);
    end;

	if OpenDialog1.Execute then begin
    	ParLabel1.Caption:='';
        TempGame.CopyDataFrom(PlayGame);
        filename:=OpenDialog1.FileName;
        PlayGame.Clear;
        PlayGame.Load(filename);
        NonFixStringedGame.CopyDataFrom(PlayGame);
        ClearAnswers;
        ...
        if not FindStartLocation then begin
        	Button1.Visible:=false;
        	SDTRichEdit.Text:='Сначала загрузите квест.';
    	end else begin
    		Form6.Caption:='Quest Player -  ' + '< '+Filename+' >';
    		SDTRichEdit.Text:='Квест загружен. Можно начинать играть.';
...
ConfigFilePath:=GetCurrentDir + '\' + 'path.cfg';
PlayGame:=TGame.Create(GameContent.PM(m));
NonFixStringedGame:=TGame.Create(GameContent.PM(m));
Randomize;
...
procedure TForm6.Label1MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
      	CheckedAnswer:=1;
	ProcessCheckAnswer;
...
procedure TForm6.Label4MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
      	CheckedAnswer:=4;
	ProcessCheckAnswer;
...
procedure TForm6.Label8MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
      	CheckedAnswer:=8;
	ProcessCheckAnswer;
...
begin
        ...
        str
              :=
                    inputbox('Quest Player',
                             'Введите ваше имя','');


        dayspassed:=0;
        PlayGame.CopyDataFrom(NonFixStringedGame);
        if str<>'' then PlayGame.RRanger.Text:=str;

        PlayGame.QuestDescription.Text:=FixStringParameters(PlayGame.QuestDescription.Text,false);
        ShowMessage(trim(PlayGame.QuestDescription.Text));
