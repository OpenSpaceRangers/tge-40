...
const MaxLocationDescriptions=10;
...
type TLocation=class
    ...
    StartLocationFlag:boolean;   //Флаг, говорящий о том, что локация
                                 //является ствртовой
    EndLocationFlag:boolean;     //Флаг, говорящий о том, что локация
                                 // является конечной
    FailLocationFlag:boolean;    //Флаг, говорящий о том, что локация
                                 //является провальной
    ...
    procedure FindLocationDescription(panel:boolean);
    function ValueOfLocationDescriptions:integer;
end;

implementation

procedure TLocation.InitMessagePointers(var MessagesValue:integer;var MessageTypes:AInt;var MessagePointers:APWS);
var i:integer;
begin
   if not VoidLocation then begin
    for i:=1 to MaxLocationDescriptions do begin
     if trim(LocationDescriptions[i].Text)<>'' then begin
        inc(MessagesValue);
        MessageTypes[MessagesValue]:=LocationTextMSGType+1000*LocationNumber;
        MessagePointers[MessagesValue]:=@LocationDescriptions[i].Text;
     end;
    end;
   end; 
     for  i:=1 to maxparameters do
     DPars[i].InitMessagePointers(MessagesValue,MessageTypes,MessagePointers);
...
begin
      answer:=false;
       ...
       for i:=1 to MaxLocationDescriptions do begin
            if trim(LocationDescriptions[i].Text)
                 <>
               trim(source.LocationDescriptions[i].Text) then goto no;
       end;
       if RandomShowLocationDescriptions
           <>
          source.RandomShowLocationDescriptions then goto no;
