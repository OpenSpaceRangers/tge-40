...
procedure TParameterDelta.InitMessagePointers(var MessagesValue:integer;var MessageTypes:AInt;var MessagePointers:APWS);
begin
      if trim(CriticalMessage.Text)<>'' then begin
        inc(MessagesValue);
        MessageTypes[MessagesValue]:=CustomCriticalMSGTYpe;
...
begin
     ...
     if (not ValuesGate.IsEqualWith(source.ValuesGate))
         or
