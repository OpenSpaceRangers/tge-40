...
procedure TParameter.InitMessagePointers(var MessagesValue:integer;var MessageTypes:AInt;var MessagePointers:APWS);
var i:integer;
begin
   ...
   if Enabled then begin
        ...
        inc(MessagesValue);
        MessageTypes[MessagesValue]:=EditorParNameMSGType;
        MessagePointers[MessagesValue]:=@Name.Text;

      for i:=1 to ValueOfViewStrings do begin
        if trim(ViewFormatStrings[i].str.Text)<>'' then begin
        	inc(MessagesValue);
        	MessageTypes[MessagesValue]:=ParViewStringMSGType;
        	MessagePointers[MessagesValue]:=@ViewFormatStrings[i].str.Text;
        end;
      end;

       if ParType<>NoCriticalParType then begin
        if trim(CriticalMessage.Text)<>'' then begin
        	inc(MessagesValue);
        	MessageTypes[MessagesValue]:=StandartCriticalMSGType;
        	MessagePointers[MessagesValue]:=@CriticalMessage.Text;
        end;
       end;
...
begin
     ...
     for i:=1 to ValueOfViewStrings do begin
        ...
        if (value>=ViewFormatStrings[i].min)
            and
           (value<=ViewFormatStrings[i].max)

           then answer:=trim(ViewFormatStrings[i].str.Text);
     end;
...
begin
    ...
    if (ParType<>NoCriticalParType)
       and
       (ParType<>SuccessParType)
       and
        LoLimit then answer:=answer+1;
...
begin
    ...
    if (ParType<>NoCriticalParType)
       ...
       (ParType<>SuccessParType)
       and
     not LoLimit then answer:=answer-1;
...
begin
     for i:=1 to maxformatviewstrings do
      ViewFormatStrings[i]:=
       TParWiewString.Create('Имя присвоенное при создании '+ inttostr(parameternumber));
