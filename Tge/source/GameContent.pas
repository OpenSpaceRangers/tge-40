...
begin
        ...
        for i:=1 to LocationsValue do Locations[i].Save_v4_0_1(@f);
        for i:=1 to PathesValue do Pathes[i].Save_v4_0_2(@f);
        close(f);
end;
...
begin
        ...
        if FileVersion>=FileVersion_3_9_6 then begin
         for i:=1 to maxparameters do Pars[i].Load_3_9_6(@f);
...
begin
       ...
       for i:=1 to LocationsValue  do begin
           ...
           current_radius:=sqrt(sqr(abs(Locations[i].screenx - x))
                                 +
                                sqr(abs(Locations[i].screeny - y)) );
